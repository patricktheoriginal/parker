...
_LIVE_MODEL         = (_load_config().get("live_model", "").strip()
                       or "models/gemini-2.5-flash-native-audio-preview-09-2025")
_CHANNELS           = 1
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"Parker: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
                    # Auto-close camera ~2s after Parker finishes speaking
