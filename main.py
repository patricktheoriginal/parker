...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Turns hand-gesture camera control on or off. When on: twisting "
            "the wrist right (like turning a doorknob) skips to the next "
            "track, twisting left goes to the previous track; gradually "
            "opening the hand raises the volume and gradually closing it "
            "(making a fist) lowers the volume, tracked continuously as the "
            "hand opens/closes -- on whatever music player is active, no "
            "voice command needed for those. Shows a small camera preview "
            "window while active. OFF by default (uses the camera "
            "continuously while on). Use when the user says 'turn on "
            "gesture control', 'enable hand gestures for music', 'turn off "
