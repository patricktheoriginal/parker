...
class HudCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "PARKER", parent=None):
        ...
        # ── Performance caches ───────────────────────────────────────────────
        self._grid_px: QPixmap | None = None          # static background grid
        self._grid_key: tuple = ()                     # (w, h, colour) it was built for
        self._face_cache: QPixmap | None = None        # last scaled face
        self._face_cache_sz: int = -1                  # size the cache was built at
        self._fps_speaking = 16                        # ~60 fps while active
        self._fps_idle     = 33                        # ~30 fps when idle — halves CPU
        self._cur_interval = self._fps_speaking

        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
        self._tmr.start(self._cur_interval)
    ...
    def _step(self):
        ...
        # Adaptive frame rate: full speed only while speaking; ease off when idle.
        want = self._fps_speaking if self.speaking else self._fps_idle
        if want != self._cur_interval:
            self._cur_interval = want
            self._tmr.start(want)
    ...
    def paintEvent(self, _):
        ...
        # grid dots — static, so render once into a cached pixmap and blit each frame
        grid_key = (W, H, C.PRI_GHO)
        if self._grid_px is None or self._grid_key != grid_key:
            self._grid_key = grid_key
            gpx = QPixmap(W, H)
            gpx.fill(Qt.GlobalColor.transparent)
            gp = QPainter(gpx)
        ...
        # face — SmoothTransformation is expensive; cache the scaled pixmap and
        # only re-scale when the integer target size actually changes.
        if self._face_px:
            fsz = int(fw * 0.62 * self._scale)
            if self._face_cache is None or self._face_cache_sz != fsz:
                self._face_cache = self._face_px.scaled(
                    fsz, fsz,
                    Qt.AspectRatioMode.KeepAspectRatio,
                    Qt.TransformationMode.SmoothTransformation,
                )
                self._face_cache_sz = fsz
...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        """)
        # Cap the document so a long session can't grow the log unbounded —
        # every insert re-lays-out the whole document, so trimming old lines
        # keeps typing snappy no matter how long the app has been running.
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "parker"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("parker:"): self._tag = "ai"
        elif tl.startswith("file:"):                             self._tag = "file"
        elif "err" in tl:                                        self._tag = "err"
        else:                                                    self._tag = "sys"
        self._tmr.start(16)

    # Insert several characters per tick instead of one-per-6ms. This keeps the
    # typewriter feel but cuts main-thread layout/scroll work by ~10×, which is
    # the main source of log lag on long replies.
    _CHARS_PER_TICK = 3

    def _step(self):
        if self._pos < len(self._text):
            end = min(self._pos + self._CHARS_PER_TICK, len(self._text))
            ...
            self._pos = end
            # Only scroll to the newest text once per tick, not once per char.
...
class FileDropZone(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # The marching-ants border only needs to move while the user is hovering
        # or dragging — keep the timer idle otherwise instead of repainting 25×/s.
    ...
    def _set_anim(self, on: bool):
        if on and not self._anim_tmr.isActive():
            self._anim_tmr.start(40)
        elif not on and self._anim_tmr.isActive():
            self._anim_tmr.stop()
    ...
    def dragEnterEvent(self, e: QDragEnterEvent):
        if e.mimeData().hasUrls():
            ...
            self._drag_over = True; self._set_anim(True); self._canvas.update()

    def dragLeaveEvent(self, e):
        self._drag_over = False; self._set_anim(self._hovering); self._canvas.update()

    def dropEvent(self, e: QDropEvent):
        self._drag_over = False
        self._set_anim(self._hovering)
    ...
    def enterEvent(self, e):
        self._hovering = True; self._set_anim(True); self._canvas.update()

    def leaveEvent(self, e):
        self._hovering = False; self._set_anim(self._drag_over); self._canvas.update()
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for Parker", str(Path.home()),
            "All Files (*.*);;"
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="Parker", user_name="",
    ...
    def _save(self):
        name = self._name_input.text().strip() or "Parker"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Parker actions."""
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "Parker").strip()
        ...
        self.on_interrupt      = None   # callable: () -> None — stop Parker mid-speech
        self._muted            = False
        self._current_file: str | None = None
    ...
    def _build_app_icon(out_path: Path) -> bool:
        """
        Render an arc-reactor style icon at 4× resolution and downsample
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "Parker") or "Parker",
            cfg.get("user_name", ""),
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "Parker") or "Parker"
